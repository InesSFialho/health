                ...
                <div class="row">
                    <div class="col">
                        ...
                        @php
                            $total_payments= 0;
                        @endphp
                        @if(!$work->already_payed($category->id)->isEmpty())
                        <h6 class="card-title"><strong>{{ $category->name }} </strong></h6>
                        <table class="table table-striped table-responsive">
                            <thead>
                                <tr>
                                    <td class="text-nowrap" scope="col" style="width: 15%">{{ __('Supplier') }}</td>
                                    <td class="text-nowrap" scope="col" style="width: 15%">{{ __('Invoice') }}</td>
                                    <td class="text-center text-nowrap" scope="col" style="width: 10%">{{ __('Invoice Date') }}</td>
                                    <td class="text-center text-nowrap" scope="col" style="width: 20%" >{{ __('Type') }}</td>
                                    <td class="text-right text-nowrap"  scope="col"style="width: 15%">{{ __('Line Value with IVA') }}</td>
                                    <td class="text-center text-nowrap"  scope="col"style="width: 10%">{{ __('Payment Date') }}</td>
                                    <td class="text-right text-nowrap"  scope="col"style="width: 15%">{{ __('Payed Value') }}</td>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($work->already_payed($category->id) as $invoiceline)
                                <tr>
                                    <td>
                                    ...
                                    <td class="text-center text-nowrap">{{$invoiceline->date}}</td>
                                    <td class="text-center text-nowrap" scope="row">{{$invoiceline->producttype}}</td>
                                    <td class="text-right text-nowrap">{{number_format($invoiceline->applied_price_with_iva,2)}}&nbsp;€</td>
                                    <td class="text-center text-nowrap">{{$invoiceline->payment_date}}</td>
                                    <td class="text-right text-nowrap">{{number_format($invoiceline->payment,2)}}&nbsp;€</td>
                                </tr>
                                @php
                                    $total_payments += $invoiceline->payment;
                                @endphp
                                @endforeach
                            </tbody>
                            <tfoot>
